...
public class PostScoreRefreshJob implements Job , CommunityConstant {
    ...
    static {
        try {
            epoch = new SimpleDateFormat("yyyy-HH-dd HH:mm:ss").parse("2022-2-1 00:00:00");
...
//        点赞数量
        long likeCount = likeService.entityLikeCount(ENTITY_TYPE_POST, postId);

        // 计算权重
        double w = (wonderful ? 75 : 0) + commentCount * 10 + likeCount * 2;
        // 分数 = 帖子权重 + 距离天数
        double v = Math.log10(Math.max(w, 1))
                + (discussPost.getCreateTime().getTime() - epoch.getTime()) / (1000 * 3600 * 24);
//        更新帖子分数
        discussPost.setScore(v);
        discussPostService.updateScore(postId,v);
//        同步搜索数据
